...
--[[
    ...
    freely, subject to the following restrictions:
       ...
       claim that you wrote the original software. If you use this softwarein a 
...
local mfloor, mceil, matan2 = math.floor, math.ceil, math.atan2

Shadowcaster = {}
Shadowcaster.__index = Shadowcaster
...
function Shadowcaster:new(isVisible, onVisible)
    return setmetatable({
        isVisible = isVisible,
        onVisible = onVisible,
    }, Shadowcaster)
...
function Shadowcaster:castLight(x0, y0, radius, start_angle, last_angle)
        ...
        for x = 1,radius do
            ...
            for i = #views,1,-1 do
                ...
                if steep[3] > steep[1] then
                    ...
                    yi = mfloor( steep[2] + steep_slope*(x-steep[1]) )
                else
                    yi = x
                end
                ...
                if shallow[4] > shallow[2] then
                    ...
                    yf = mfloor( shallow[2] + shallow_slope*(x-shallow[1]) )
                else
                    yf = 0
                end
                ...
                for y = yi,yf,-1 do
                    ...
                    if arc_angle >= tau or arc_angle >= (matan2(ty,tx)-start_angle) % tau then
                        -- if math.pow(tx, 2) + math.pow(ty, 2) < math.pow(radius, 2) then
                        self.onVisible( x0+tx,y0+ty )
                        -- end
                    end
                    -- Found a blocking cell
                    if not self.isVisible( x0+tx,y0+ty ) then
...
return setmetatable(Shadowcaster, {
    __call = Shadowcaster.new
})
